...
@Model.register("cvt_tagger")
class CvtTagger(Model):
    """
    The ``cvt_tagger`` encodes a sequence of text with an optional 
    ``Seq2SeqEncoder``, then uses a softmax model to predict a tag for each 
    token in the sequence (Greedy decoding). This is the same as the 
    ``simple_tagger`` but with the following differences:
    
    1. It allows you to not have to use a ``Seq2SeqEncoder``
    2. It allows you to have the option of asking for predictions from the 
       forward, backward, future, and past encoders.
    ...
    def forward(self,  # type: ignore
        ...
        class_probabilities: ``torch.FloatTensor``
            ...
            representing a distribution of the tag classes per word.
        mask : ``torch.LongTensor``
            The text field mask for the input tokens
        tags : ``List[List[int]]``
            The predicted tags based on the softmax scores (Greedy decoding)
